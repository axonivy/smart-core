...
import static ch.ivyteam.smart.core.tool.impl.FormSchemaTool.NAME;
...
@ManagedServer
public class FormSchemaToolTest {

  @RegisterExtension
  McpClientAccess mcp = new McpClientAccess();

  @Test
  void callTool(McpAsyncClient client) throws Exception {
    var result = client.callTool(CallToolRequest.builder().name(NAME).build()).block();

    assertThat(result.content()).hasSize(2);
    assertThat(((TextContent) result.content().get(0)).text()).isEqualTo(FORM.schema().toString());
