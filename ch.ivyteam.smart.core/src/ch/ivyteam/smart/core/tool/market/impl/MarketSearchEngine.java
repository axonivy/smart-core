...
public class MarketSearchEngine {
  ...
  private static WebTarget market() {
    // https://market.axonivy.com/stable/api/product?type=all&keyword=&language=en
  ...
  public static Optional<ArrayNode> enrich(JsonNode json) {
    var products = Optional.ofNullable(json)
    ...
    if (id != null) {
      if (product.get("_links") instanceof ObjectNode links) {
        if (links.get("self") instanceof ObjectNode self) {
          ...
          self.put("versions", MARKET_API + "/product/" + id.asText() + "/versions?designerVersion=&isShowDevVersion=true");
        }
      }
    }
  }
